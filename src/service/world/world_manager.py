...
class WorldManager:
    ...
    def resolve_action(
        ...
        *,
        ...
        if action.action == AgentActionType.TALK:
            return self._talk(agents, actor, action)
    ...
    def apply_passive_needs(
        ...
        for agent in agents.values():
            agent.needs["hunger"] = min(
                100,
                agent.needs.get("hunger", 0) + self.rules.hunger_per_tick,
            )
            agent.needs["energy"] = max(
                0,
                agent.needs.get("energy", 0) - energy_cost,
            )
    ...
    def _move(
        ...
        return {
            ...
            "target_id": target,
            "payload": {"from": previous, "to": target, "reason": action.reason},
        }
    ...
    def _talk(
        ...
        actor.relationships[target.agent_id] = min(
            100,
            actor.relationships.get(target.agent_id, 0) + 1,
        )
        target.relationships[actor.agent_id] = min(
            100,
            target.relationships.get(actor.agent_id, 0) + 1,
        )
    ...
    @staticmethod
    def _next_location(world: WorldState, current: str) -> str:
